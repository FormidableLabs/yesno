...
However usually when writing unit tests we don't want our requests to hit any external services, but we still want to validate the correct request _would_ be generated. In this case we can call `yesno.mock()`, which will intercept generated HTTP requests and respond with a provided mock response.
...
When you `mock` requests you must provide YesNo with mocks for all expected requests.

```javascript
const { yesno } = require('yesno-http');
const { expect } = require('chai');
const myApi = require('./fb-api');
...
describe('api', () => {
  ...
  it('should get users', async () => {
    ...
    await myApi.getUsers(); // Error! We only have mocks defined for 1 request, YesNo won't find a match here.
  });
});
```
...
```javascript
yesno.spy();
apiKey = await myApi.authenticate();
await myApi.getUsers(apiKey);

yesno.matching(/example.com\/auth/).redact('response.body.token');
yesno.matching(/example.com\/users/).redact('request.headers.authorization');
